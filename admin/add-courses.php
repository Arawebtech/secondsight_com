...
$final_price = floatval($_POST['price']);
    $price = $price + $gst_amount;

if ($mrp > 0 && $final_price <= $mrp) {
    $discount_percentage = round((($mrp - $final_price) / $mrp) * 100, 2);
}
	...
	$sql_query = "INSERT INTO courses (s_name, s_title, url, short_name, rating, price, gst_percentage, gst_amount, final_price, discount_percentage, duration, short_description, description, validity, duration_time, banner_image, thumbnail_image, alt, meta_keyword, meta_description, s_schema, status, instructor_name, inst_about, inst_img, community_links, created_date, created_by, updated_date) 
    VALUES ('$s_name', '$s_title', '$url', '$short_name', '$rating', '$price', '$gst_percentage', '$gst_amount','$final_price', '$discount_percentage', '$mrp', '$short_description', '$description', '$validity', '$duration_time', '$banner_image', '$thumbnail_image', '$alt', '$meta_keyword', '$meta_description', '$s_schema', '$status', '$instructor_name', '$inst_about', '$inst_img', '$community_links', '$created_date', '', '')";
...
if (isset($_POST['submit']) && $_POST['submit'] == 'Update') {
    ...
    // PRICE + GST SECTION (FIXED)
    $price = floatval($_POST['price']);   // Base price
    $mrp = floatval($_POST['mrp']);       // MRP
    $gst_percentage = floatval($_POST['gst_percentage']);
    ...
    $duration_time = addslashes($_POST['duration_time']);
    $short_description = addslashes($_POST['short_description']);
    $description = addslashes($_POST['description']);
    $validity = addslashes($_POST['validity']);
    ...
    if (!empty($_FILES["banner_image"]["name"])) {
        ...
        move_uploaded_file($_FILES["banner_image"]["tmp_name"], "../assets/img/course-img/" . $banner_image);
    } else {
    ...
    if (!empty($_FILES["thumbnail_image"]["name"])) {
        ...
        move_uploaded_file($_FILES["thumbnail_image"]["tmp_name"], "../assets/videothumbnail/" . $thumbnail_image);
    } else {
    ...
    if (!empty($_FILES["inst_img"]["name"])) {
        $inst_img = $_FILES["inst_img"]["name"];
        move_uploaded_file($_FILES["inst_img"]["tmp_name"], "../assets/img/instructors/" . $inst_img);
    } else {
...
<body class="hold-transition skin-blue sidebar-mini">
    <div class="wrapper">
       ...
       value="<?php 
            if ($isEdit) {
                echo isset($info_blog->final_price) ? $info_blog->final_price : '';
            } else {
                echo isset($info_blog->price) ? $info_blog->price : '';
            }
       ?>"
    ...
    <div class="form-group">
        ...
        <div class="input-group">
            ...
            <input type="number" id="gst_percentage" name="gst_percentage" step="0.01"
                value="<?php echo isset($info_blog->gst_percentage) ? $info_blog->gst_percentage : '0'; ?>"
                class="form-control" onchange="calculateDiscount()">
        </div>
    </div>
</div>
